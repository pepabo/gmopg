...
test('.searchTradeMulti calls API and returns response - Paypay', async t => {
  const expect: SearchTradeMultiPaypayResult = {
    Status: Status.Capture,
    ProcessDate: 'processdate',
    JobCd: JobCd.Auth,
    AccessID: 'accessid',
  ...
  const args = {
    ...
    ShopPass: 'shoppass',
    OrderID: 'orderid',
    PayType: PayType.Paypay,
  }
  const res = await multiTranable.searchTradeMulti<SearchTradeMultiPaypayResult>(args)
